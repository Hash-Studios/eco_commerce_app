...
//  List<int> whichWidget = [1, 2, 3,2, 4, 5, 3, 6, 5,7];

  List<int> whichWidget=[1,2,3,2,4,5,6,5,3,7,1,2,1,2,3,2,5,6,5,3,7,1,2,2,3,2,5,6,5,3,7,1,2,2,3,2,5,6,5,3,7,1,2,2,3,2,5,6,5,3,7,1,2,2,3,2,5,6,5,3,7,1,2];
  List<ItemWidget> items;
  double height;
  double width;

//  Widget textBanner;
//  Widget gradientBanner;
//  Widget userGradientBanner;
//  Widget categoriesBanner;
//  Widget emptySizedBoxBanner;
    ...
    return Container(
      ...
      child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return ListTile(
              title: (whichWidget[index] == 1)
                  ? item.landingSliderWidget(context, header: [])
                  : (whichWidget[index] == 2)
                  ? item.popularSliderWidget(context,
                  header: [(index>2)?_userGradientBanner():_textBanner("Popular Now")])
                  : (whichWidget[index] == 3)
                  ...
                  context, widget.products.sublist(1, 6),
                  header: [_emptyBoxBanner(height: 30),_textBanner("Trending Products")])
                  : (whichWidget[index] == 6)
                  ? item.subListWidget(context, header: [])
                  : item
                  .zoomSlideWidget(context, header: [_textBanner("Disposables")]),);
          }),
    );
  }
